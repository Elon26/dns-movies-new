...
- Открыть терминал.
- Ввести в терминал и выполнить команду docker compose up postgres
...
- Ввести в терминал и выполнить команду docker compose --profile dev up
- Дождаться завершения процесса, т.е. пока в консоли не перестанут создаваться новые строки.
- Открыть приложение в браузере по адресу http://localhost:3000/
- ВНИМАНИЕ. Если возникает проблема, связанная с тем, что запросы на сервер остаются в статусе ожидания (см. вкладку Сеть в панели разработчика), то необходимо заново остановить выполнение процесса (при помощи команды ctrl + C), заново его запустить (при помощи команды docker compose --profile dev up) и обновить страницу браузера.
